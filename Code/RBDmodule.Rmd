...
# Importing the HI 2 network
load("/Volumes/Taveluz_1T/Universidad/11 Semestre/Thesis/Thesis_R/Data/HI_2_networks.RData")

# Importing the file
RBDlist <- as.character(unlist(read_delim("/Volumes/Taveluz_1T/Universidad/11 Semestre/Thesis/Thesis_R/Data/Files_ready/RBD_list.txt", "\t", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)))
...
# GO biological enrichment of RBD nodes with NEAT.  In UNDIRECTED networks, A and B can be inverted – you can do this, because you just swap the degrees of A and B in the hypergeometric (so, you invert n and K), and an hypergeometric with parameters n, K, N is the same as an hypergeometric K,n,N. That is why the RBD list of seed genes is in blist and the ist of GOBP in alist
...
RBDrich$padj <- p.adjust(RBDrich$p, method = 'BH')
...
# Importing the DIAMOnD prediction
prediction_interactome_2_500 <- read_delim("/Volumes/Taveluz_1T/Universidad/11 Semestre/Thesis/Thesis_R/Data/Files_ready/prediction_interactome_2_500.txt",
                                       "\t",
                                       escape_double = FALSE,
                                       trim_ws = TRUE)
names(prediction_interactome_2_500) <- c("Rank", "Node")
...
# Validating the first 500 DIAMOnD nodes with GO biological process enrichment for the terms
...
The observed clustering coefficient of the RBD is very far awasy in term of standard deviations (38.47 $\sigma$) from the mean of the random expectation. 
...
The observed average paths length are significantly (z-score = ) smaller than the random expectation for network drawn with an edge probability of 
